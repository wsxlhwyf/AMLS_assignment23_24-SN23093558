...
class mydata(torch.utils.data.Dataset):
    def __init__(self, x, y):
        self.x = torch.FloatTensor(x).view(-1, 3, 28, 28) # Transfer the data shape into [batch, channel，length， width]
        self.y = torch.nn.functional.one_hot(torch.LongTensor(y), num_classes=9).float().view(-1, 9)   # Transfer the shape of y into [batch, 9]
...
class CNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.convd1 = torch.nn.Conv2d(in_channels=3, out_channels=32, kernel_size=(3,3), stride=1, padding='valid')
        self.pool1 = torch.nn.MaxPool2d(kernel_size=3, stride=1)
        self.convd2 = torch.nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3,3), stride=1, padding='valid')
        self.pool2 = torch.nn.MaxPool2d(kernel_size=3, stride=1)
        self.convd3 = torch.nn.Conv2d(in_channels=64, out_channels=32, kernel_size=(3,3), stride=1, padding='valid')
        self.pool3 = torch.nn.MaxPool2d(kernel_size=3, stride=1)
        self.convd4 = torch.nn.Conv2d(in_channels=32, out_channels=3, kernel_size=(3,3), stride=1, padding='valid')
        self.pool4 = torch.nn.MaxPool2d(kernel_size=3, stride=1)
        self.mlp1 = torch.nn.Linear(432, 9) # Fully connected layer
...
def train_and_eval(model, criterion, opt, epochs, train_dataloader, val_dataloader, test_dataloader):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_dataloader):
            ...
            train_pre += torch.max(pre, -1)[1].cpu().detach().numpy().tolist()
            train_label += torch.max(y,-1)[1].cpu().detach().numpy().tolist()
        ...
        for _, (x, y) in enumerate(val_dataloader):  
            ...
            val_pre += torch.max(pre,-1)[1].cpu().detach().numpy().tolist()
            val_label += torch.max(y, -1)[1].cpu().detach().numpy().tolist()
        ...
        for _, (x, y) in enumerate(test_dataloader):  
            ...
            test_pre += torch.max(pre,-1)[1].cpu().detach().numpy().tolist()
            test_label += torch.max(y, -1)[1].cpu().detach().numpy().tolist()
...
# Dataprocessing
print('start...')
pndata = np.load('./Datasets/pathmnist.npz')
